...
from gui.controllers import activation_controller, navigation_controller
...
def setup_callbacks(app) -> None:
    ...
    app.cnn_radio.config(command=lambda: activation_controller.populate_layer_selector(app))
    app.fc_radio.config(command=lambda: activation_controller.populate_layer_selector(app))
    app.layer_selector.bind("<<ComboboxSelected>>", lambda event: activation_controller.update_all_visuals(app))
    ...
    app.index_entry.bind("<Return>", lambda event: activation_controller.update_all_visuals(app))

    app.grid_checkbutton.config(command=lambda: activation_controller.update_input_panel(app))

    canvas = app.activation_panel.canvas
    canvas.bind("<Button-1>", lambda event: activation_controller.on_grid_click(app, event))
    ...
    app.detail_panel.canvas.bind("<Configure>", lambda event: activation_controller.on_detail_canvas_resize(app, event))
    app.detail_grid_checkbutton.config(command=lambda: activation_controller.on_detail_canvas_resize(app, None))
